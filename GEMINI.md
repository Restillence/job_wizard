...
---
## 2.1. Strict Agent & Testing Constraints (ANTI-CHEAT RULES)

To ensure the system operates reliably in production, the following rules are ABSOLUTE and must never be violated during development or testing:

* **NO MOCKING EXTERNAL SERVICES:** Do not use mock data, hardcoded arrays (e.g., injecting specific companies into the DB to bypass discovery), or fake API responses unless explicitly instructed by the user.
* **NO BYPASSING:** If a dynamic external call (like Tavily, Serper, or Crawl4AI) fails or returns 0 results, DO NOT hardcode fallback data to make the rest of the script work. Your job is to debug and fix the actual query or LLM extraction prompt.
* **FAIL LOUDLY:** If Step A (Discovery) fails, the script MUST crash or return an error. Do not silently proceed to Step B (Extraction) with fake data. 
* **RADICAL TRANSPARENCY (LOGGING):** When debugging external APIs or LLMs, you must implement extensive `print()` logging in the test scripts. Always print:
    1. The exact raw search string being sent to the API.
    2. The raw JSON response received from the Search API BEFORE passing it to the LLM.
    3. The exact system prompt and user prompt sent to the LLM.
    4. The raw output returned by the LLM.
* **TEST-DRIVEN DEVELOPMENT (ISOLATION):** When tasked with fixing a pipeline step, modify the underlying service logic (e.g., `job_discovery.py`), NOT the test script itself, to force a passing result.

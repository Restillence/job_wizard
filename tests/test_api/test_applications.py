...
def test_draft_application(client):
    ...
    # Create Job
    job = Job(id=str(uuid.uuid4()), source_url="http://test.com", title="Dev", company="Test Inc", description="Desc")
    db.add(job)
    ...
    with patch("src.api.routers.applications.cover_letter_service.generate_draft") as mock_gen:
...
def test_approve_application(client):
    ...
    job = Job(id=str(uuid.uuid4()), source_url="http://test2.com", title="Dev", company="Test Inc", description="Desc")
    db.add(job)
    
    app = Application(id=str(uuid.uuid4()), user_id=user_id, job_id=job.id, status=ApplicationStatus.Drafted)
    db.add(app)
    db.commit()
